...
# Version 2: Assume no circles or crosses on the terrain
...
# Exclude the neighbors such that the box is standing on a orange tile
def neighbors(Tiles, Orange, box):
    neighbor = []
    for i in range(4):
...
def BFS(Tiles, Orange, box, D):
    ...
    while len(visited_vertices) != 0:
        ...
        box = Box(s, v[0], v[1])
        for node in neighbors(Tiles, Orange, box):
...
# Import the level txt file and store them as list of coordinates as stated in the instruction
# Tiles is the list of all tiles that are not '-' while Orange stores the tiles that look like '.'
...
if None not in {Start, Destination}:
    ...
    prev = BFS(Tiles, Orange, box, Destination)
